...
$queryPedido = mysqli_query($conexion, $infoPedido);
...
class MYPDF extends TCPDF
{
    ...
    {
        $image_file = dirname(__FILE__) . '/img/logo.png';
        $this->Image($image_file, 30, 20, 15, '', 'PNG', '', 'T', false, 300, '', false, false, 0, false, false, false);
...
// Informacion del pdf
$pdf->SetCreator('fernando');
$pdf->SetAuthor('wilches');
...
$queryCarrito   = mysqli_query($conexion, $sqlCarritoCompra);
...
while ($dataP = mysqli_fetch_array($queryCarrito)) {
    ...
    $html .= '
    <tr>
    <td style="font-size:9px; border-right: 1px solid #666; border: 0.2px solid #666;">' . $dataP['tipo_extintor'] . '</td>
